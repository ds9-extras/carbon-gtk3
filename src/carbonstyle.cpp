        ...
        bool isMaximized )
    {
        ...
        if( window || widget )
        {
            ...
            if( !Gtk::gdk_map_to_toplevel( window, widget, &wx, &wy, &ww, &wh, true ) )
            {
                ...
                #if CARBON_DEBUG
                std::cerr << "Carbon::Style::renderWindowBackground - map_to_toplevel failed" << std::endl;
...
//         }
        ...
        const ColorUtils::Rgba base( _settings.palette().color( group, Palette::Button ) );
            ...
            if(sh && sw)
            {
                ...
                if( windowStrings && windowStrings[0] )
                {
                    ...
                    if( font.isValid() && layoutWidth>0 )
                    {
                        ...
                        cairo_set_source( context, _settings.palette().color( group, Palette::WindowText ) );
